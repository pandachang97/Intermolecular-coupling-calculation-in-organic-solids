...
program g09_tq
    ...
    real(8), allocatable :: Gross_orb_pop(:)
...
!   warn = .true.
    do b = 1, g09_task_numBasisFunctions
        ...
        if ( b /= c ) then
        Gross_orb_pop(b) = Gross_orb_pop(b) + tdm(b,c) * overlap(b,c) * 5.d-1
        else
        Gross_orb_pop(b) = Gross_orb_pop(b) + tdm(b,c) * overlap(b,c) 
        endif
        end do
    end do
...
!   tq = tq * dsqrt(2.d0)
    ...
    write(fno,*) "The Mulliken transition charges"
...
subroutine tq_init(fch, logf, fout, estate )
    ...
    if ( makeinput ) then
        ...
        write( fno, * ) '%NProcShared=24'
        write( fno, * ) '%mem=32GB'
        write( fno, * ) '# '//trim(emethod)//' SP IOP(9/40=5) IOP(3/33=1)' &
                        //' Pop=Full NoSymm iIOP(6/22=-4) IOP(6/29=1) '
        write( fno, * )
...
subroutine g09_tdm( logf )
    ...
    logical :: exists, special
    character(100) read_line
    integer  :: bol(5), row, i, j, m, n, pos, nimei
...
!               112 continue
                ...
                if ( maxval( bol ) == g09_task_numBasisFunctions ) exit
            end do
            exit
        end if
    end do
