    ...
    do
        ...
        if ( index( read_line , '*** Overlap ***' ) .ne. 0 ) then
            do 
                ...
                col = 0
                nimei = 0 
                read( fno, '(5x,4(I12,2x),(I12,1x))', end = 102, iostat = nimei ) col(:) 
!               print*, 'The IO message is', nimei
                if ( nimei /= 0 ) stop 'Error reading colomn of overlap.'
!               write (*,*) col(:)
                102 continue
                ! read the info in each row
                do
                    read ( fno, '(I7, 5E14.6)', end=103, iostat = nimei ) row,   &
                        (overlap( row, col(i) ), i = 1,           &
                             maxval( col(:) - col(1) ) + 1 )
                if ( nimei /= 0 ) stop 'Error reading elements of overlap.'
!               write (*,*) row
                    103 continue
