...
program g09_ctint
    ...
    logical            :: intra_chain_CT
    integer            :: cut_H_m1, cut_H_m2, cut_H_basis_m1, cut_H_basis_m2
    integer            :: m1_line, m2_line 
    real*8             :: Nor_coe


    ! initialize the program, read options and input file
   ! intra_chain_CT = .false.

    call ctint_init(fch_m1, log_m1, fch_m2, log_m2, &
                    fch_d, log_d, fout, intra_chain_CT, &
                    cut_H_basis_m1, cut_H_basis_m2 )
    print*, intra_chain_CT
    ...
    m1moc = moc
        if( intra_chain_CT ) then
        homo1 = homo1 - 1
        lumo1 = homo1 + 1
        baseN1 = baseN1 - cut_H_basis_m1
        baseN1u = baseN1u - cut_H_basis_m1
        deallocate( m1moc )
        allocate( m1moc( baseN1, baseN1u ) ) 
        do i = 1 , baseN1
            do j = 1 , baseN1u
...
! normalization
        ...
        Nor_coe = 0.d0
            do i = 1 , baseN1
            Nor_coe = Nor_coe + ( m1moc( i, j ) )**2
            enddo
        Nor_coe = dsqrt( Nor_coe )
        m1moc( :, j ) = m1moc( :, j ) / Nor_coe
        enddo
        endif
  ! print*, 'molecule 1 basis set size', baseN1, baseN1u
    ...
    m2moc = moc
        ...
        homo2 = homo2 - 1
        lumo2 = homo2 + 1
        baseN2 = baseN2 - cut_H_basis_m2
        baseN2u = baseN2u - cut_H_basis_m2
        deallocate( m2moc )
        allocate( m2moc( baseN2, baseN2u ) ) 
        do i = 1 , baseN2
            do j = 1 , baseN2u
  ...
  !! Normalization
        ...
        Nor_coe = 0.d0
            do i = 1 , baseN2
            Nor_coe = Nor_coe + ( m2moc( i, j ) )**2
            enddo
        Nor_coe = dsqrt( Nor_coe )
        m2moc( :, j ) = m2moc( :, j ) / Nor_coe
        enddo

        endif
  !     print*, 'molecule 2 basis sets size', baseN2, baseN2u
    ...
    write( fno, * ) 'Nonorthogonal Sub Hamiltonian (eV)'
    ...
    write( fno, * ) 'Orthogonalized Sub Hamiltonian (eV)'
    write( fno, * ) ',|HOMO1>,|HOMO2>,|LUMO1>,|LUMO2>'
    write( fno, '(a,4(f14.4))' ) '<HOMO1|',(Hsub(1,i),i=1,4)
    write( fno, '(a,4(f14.4))' ) '<HOMO2|',(Hsub(2,i),i=1,4)
    write( fno, '(a,4(f14.4))' ) '<LOMO1|',(Hsub(3,i),i=1,4)
    ...
    write( fno, * ) 'Notice that th = -<HOMO1|H|HOMO2>. But when '
    write( fno, * ) 'printing out it, the negative sign is ignored.'
...
subroutine ctint_init(fch_m1, log_m1, fch_m2, log_m2, fch_d, log_d, &
        ...
        do while ( ios == 0 )
            ...
            if ( ios == 0 ) then
                ...
                select case (label)
                    ...
                    case('intra_CT')
                        read( buff, *, iostat = ios ) intra_chain_CT
                        print*, ' intra_CT ', (( intra_chain_CT ))
                    case('cut_H_m1')
                        read( buff, *, iostat = ios ) cut_H_m1
                        print*, ' cut_H_m1 ', (( cut_H_m1 ))
                    case('cut_H_m2')
                        read( buff, *, iostat = ios ) cut_H_m2
                        print*, ' cut_H_m2 ', (( cut_H_m2 ))
                    case('cut_H_basis_m1')
                        read( buff, *, iostat = ios ) cut_H_basis_m1
                        print*, ' cut_H_basis_m1 ', ((cut_H_basis_m1))
                    case('cut_H_basis_m2')
                        read( buff, *, iostat = ios ) cut_H_basis_m2
                        print*, ' cut_H_basis_m2 ', ((cut_H_basis_m2))
                    case default
    ...
    if ( makeinput ) then
        ...
        !! A warning for the format of xyz files will be printing
        print*, 'Warning!!! In the xyz files, there will be no blank '
        print*, 'lines at the end of coordinates of the molecules. If'
        print*, 'There are any blank lines in the end, please delete '
        print*, 'them and run the execute command again.'

        ! the calculation for molecule 1
        open( unit = fno, file = trim(adjustl(task))//'_m1.com', &
              action = 'write' )
        write( fno, * ) '%RWF='//trim(adjustl(task))//'_m1'
   !    write( fno, * ) '%NoSave'
        write( fno, * ) '%Chk='//trim(adjustl(task))//'_m1'
        write( fno, * ) '%NProcShared='//trim(adjustl(nproc))
        write( fno, * ) '%mem=20GB'
        write( fno, * ) '#P SP '//trim(method)//' IOP(3/33=1) IOP(6/7=3) IOP(3/32=2) '//&
                        'Nosymm  '
        write( fno, * )
...
105     continue
        ...
        open( unit = fno, file = trim(adjustl(task))//'_m2.com', &
              action = 'write' )
        write( fno, * ) '%RWF='//trim(adjustl(task))//'_m2'
!       write( fno, * ) '%NoSave'
        write( fno, * ) '%Chk='//trim(adjustl(task))//'_m2'
        write( fno, * ) '%NProcShared='//trim(adjustl(nproc))
        write( fno, * ) '%mem=20GB'
        write( fno, * ) '#P SP '//trim(method)//' IOP(3/33=1) IOP(6/7=3) IOP(3/32=2) '//&
                        'Nosymm '
...
106     continue
        ...
        open( unit = fno, file = trim(adjustl(task))//'_d.com', &
              action = 'write' )
        write( fno, * ) '%RWF='//trim(adjustl(task))//'_d'
        write( fno, * ) '%NoSave'
        write( fno, * ) '%Chk='//trim(adjustl(task))//'_d'
        write( fno, * ) '%NProcShared='//trim(adjustl(nproc))
        write( fno, * ) '%mem=20GB'
        write( fno, * ) '#P SP '//trim(method)//' IOP(3/33=1) IOP(6/7=3) IOP(5/33=3) IOP(3/32=2) '//&
                        ' Nosymm'
        write( fno, * )
...
!       read( fno2, * )
        ...
        do
            ...
            if ( intra_chain_CT ) then
                m1_line = m1_line + 1
!               print*,'intra CT is true in monomer1'
                if ( m1_line == cut_H_m1 ) exit
            endif

            write( fno, * ) buff
        end do
107     continue
!       print*, m1_line ,'line is deleted in monomer 1'
        close( fno2 )
        open( unit = fno2, file = trim(fxyzm2), action = 'read' )
!       read( fno2, * )
!       read( fno2, * )
        ...
        do
            ...
            if ( intra_chain_CT ) then
                m2_line = m2_line + 1
!               print*,'intra CT is true in monomer2'
                if ( m2_line == cut_H_m2 ) exit
            endif

            write( fno, * ) buff
        end do
108     continue
!       print*, m2_line ,'line is deleted in monomer 2'
        ! write a blank line so g09 doesnt crash
        write( fno, * ) 
        close( fno2 )
        close( fno )
